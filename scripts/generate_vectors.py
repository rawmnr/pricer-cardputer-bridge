...
def _tagtinker_pre_transmit_gap_us(index: int, frame_count: int) -> int:
    """Return the settled gap before a frame in the TagTinker full-image flow."""
    if index in (1, 2, frame_count - 1):
        return 50_000
    if 2 <= index < frame_count - 1 and (index - 2) % 32 == 0:
        return 1_000
    return 0
...
def write_tagtinker_orientation_source(vectors: list[AirFrame]) -> None:
    ...
    entries = "\n".join(
        ...
        f"{vector.inter_repeat_gap_us}, "
        f"{_tagtinker_pre_transmit_gap_us(index, len(vectors))}}},"
        for index, (symbol, vector) in enumerate(zip(symbols, vectors, strict=True))
    )
    source = f"""#include "orientation_test.hpp"
...
// Settle gaps mirror TagTinker-reference/scenes/tagtinker_scene_transmit.c:
// 50 ms follows ping and params; 1 ms follows each non-final 32nd data
// frame; 50 ms follows the final data frame. Gaps are stored on the next frame.
